...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function makeSnippets(num) {
    ...
    for (let i = 1; i <= rows; i++) {
      for (let j = 1; j <= cols; j++) {
        source = [video.width * (1 - j / cols), video.height * (1 - i / rows), video.width / rows, video.height / cols];
  ...
  const widthHeight = [video.width / Math.sqrt(numSnippets), video.height / Math.sqrt(numSnippets)];
  ...
  scramble.addEventListener("click", () => {
    ...
    for (var key in snippets) {
      ...
      snippets[key].incX = (snippets[key].maxX - snippets[key].pos[0]) / movementSpeed;
      snippets[key].incY = (snippets[key].maxY - snippets[key].pos[1]) / movementSpeed;
    }
  });
